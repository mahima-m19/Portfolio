...
export default function AchievementsSection() {
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 md:px-8 lg:px-16 py-12 sm:py-16 lg:py-20 bg-[var(--bg-primary)] text-[var(--text-primary)] lg:ml-[22rem] overflow-hidden transition-colors duration-300">
      <div className="relative max-w-6xl w-full z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
          {achievements.map((achievement, index) => {
            ...
            return (
              <div
                key={index}
                className="group p-5 sm:p-6 bg-[var(--bg-secondary)] rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2"
                style={{ border: "2px solid var(--sidebar-border)" }}
              >
                {/* Icon and Category */}
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className="p-2.5 sm:p-3 bg-[var(--bg-hover)] rounded-xl group-hover:scale-110 transition-transform duration-300">
                    <IconComponent
                      size={20}
                      className="sm:w-6 sm:h-6 text-[var(--accent-color)]"
                    />
                  </div>
                  <span className="text-xs font-semibold text-[var(--accent-color)] bg-[var(--bg-hover)] px-2.5 sm:px-3 py-1 rounded-full">
                    {achievement.year}
                  </span>
                </div>
                ...
                {/* Category Badge */}
                <div className="inline-block px-2.5 sm:px-3 py-1 bg-[var(--bg-hover)] rounded-full text-xs font-medium">
        ...
        {/* Stats Section */}
        <div className="mt-12 sm:mt-14 lg:mt-16 grid grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
          <div
            className="text-center p-4 sm:p-5 lg:p-6 bg-[var(--bg-secondary)] rounded-xl sm:rounded-2xl"
            style={{ border: "2px solid var(--sidebar-border)" }}
          >
            <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[var(--accent-color)] mb-1 sm:mb-2">
              6+
            </div>
